  ...
  try {
    ...
    const detailedTest = await page.evaluate(async () => {
      try {
        ...
        const reactExpectedPagination =
          response.data && response.data.data && response.data.data.pagination;
        ...
        return {
          ...
          structure: {
            ...
            actualPaginationKeys: reactExpectedPagination
              ? Object.keys(reactExpectedPagination)
              : [],
          },
        };
      } catch (error) {
    ...
    if (detailedTest.success) {
      ...
      console.log(
        `      - React expected path exists: ${detailedTest.structure.reactExpectedPath}`
      );
      console.log(
        `      - React expected pagination exists: ${detailedTest.structure.reactExpectedPagination}`
      );
      console.log(`      - Actual data length: ${detailedTest.structure.actualDataLength}`);
      console.log(
        `      - Actual pagination keys: ${detailedTest.structure.actualPaginationKeys.join(', ')}`
      );
