...
test.describe('Create New Client with Logo', () => {
  test('should create a new client with logo upload', async ({ page }) => {
    ...
    const addClientButton = page.locator(
      'button:has-text("Add Client"), button:has-text("إضافة عميل"), button:has-text("Add"), button:has-text("إضافة")'
    );
    ...
    if (addButtonVisible) {
      ...
      if (modalVisible) {
        ...
        const nameEnInput = page
          .locator('input[placeholder*="Client Name"], input[placeholder*="الإنجليزية"]')
          .first();
        ...
        if (uploadAreaVisible) {
          ...
          if (fileInputExists) {
            ...
            const logoPreview = page.locator(
              'img[alt*="logo"], img[alt*="Logo"], img[alt*="عميل"]'
            );
            ...
            const saveButton = page.locator(
              'button:has-text("Save"), button:has-text("حفظ"), button[type="submit"]'
            );
