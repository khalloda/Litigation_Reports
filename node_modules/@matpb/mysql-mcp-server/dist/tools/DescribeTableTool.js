...
class DescribeTableTool extends MCPTool {
    name = "describe_table";
    ...
    async execute(input) {
        try {
            ...
            const columns = await mysqlManager.executeQuery(`DESCRIBE \`${tableName}\``);
            // Get indexes
            const indexes = await mysqlManager.executeQuery(`SHOW INDEX FROM \`${tableName}\``);
            // Get table status for additional info
            const tableStatus = await mysqlManager.executeQuery(`SHOW TABLE STATUS LIKE '${tableName}'`);
