...
test.describe('Debug Client Modal', () => {
  test('should debug what is actually in the client edit modal', async ({ page }) => {
    ...
    const editButton = clientRow.locator(
      'button:has-text("Edit"), button:has-text("تعديل"), button[title*="Edit"], button[title*="تعديل"]'
    );
    ...
    for (let i = 0; i < allInputs.length; i++) {
      ...
      console.log(
        `Input ${i + 1}: type="${type}", name="${name}", placeholder="${placeholder}", visible=${visible}`
      );
    }
    ...
    const logoElements = page.locator(
      '[class*="logo"], [id*="logo"], :has-text("logo"), :has-text("Logo"), :has-text("شعار")'
    );
    ...
    for (let i = 0; i < logoElementCount; i++) {
      ...
      console.log(
        `Logo element ${i + 1}: ${tagName}, class="${className}", text="${text}", visible=${visible}`
      );
    }
    ...
    for (let i = 0; i < fileInputCount; i++) {
      ...
      console.log(
        `File input ${i + 1}: accept="${accept}", visible=${visible}, display="${display}"`
      );
    }

    // Look for upload-related elements
    const uploadElements = page.locator(
      '[class*="upload"], [id*="upload"], :has-text("upload"), :has-text("Upload"), :has-text("رفع")'
    );
    ...
    for (let i = 0; i < uploadElementCount; i++) {
      ...
      const visible = await element.isVisible();
      console.log(
        `Upload element ${i + 1}: ${tagName}, class="${className}", text="${text}", visible=${visible}`
      );
    }
    ...
    for (let i = 0; i < Math.min(allDivs.length, 20); i++) {
      ...
      if (
        className &&
        (className.includes('upload') || className.includes('logo') || className.includes('file'))
      ) {
        console.log(
          `Div ${i + 1}: class="${className}", text="${text?.substring(0, 50)}", visible=${visible}`
        );
      }
    }
