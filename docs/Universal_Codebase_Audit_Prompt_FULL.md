...
## OBJECTIVE
Perform a **complete static analysis** of the repository and generate a comprehensive documentation bundle inside **`Cursor_Output/`**. Assume **monorepo** unless proven otherwise. Detect **all languages**, **frameworks**, **build tools**, **package managers**, **databases**, **APIs**, **frontends**, **services**, **infrastructure**, and **testing/operations** patterns. Where runtime evidence is missing, state assumptions as **clearly marked hypotheses** with confidence levels and ways to verify.

---

## GLOBAL RULES
1. **All outputs must live only under:** `Cursor_Output/` (create subfolders as needed).  
...
## REQUIRED OUTPUTS (WRITE ALL UNDER `Cursor_Output/`)
...
- **Assumption (Medium confidence):** _statement_  
  **Why:** _what hints you saw_  
  **How to verify:** _commands/paths/tests to run_
...
8. **Produce all deliverables** under `Cursor_Output/` with diagrams, tables, citations, and clear recommendations.
...
- Count of files produced under `Cursor_Output/`.  
