  ...
  try {
    ...
    const pageAnalysis = await page.evaluate(() => {
      ...
      links.forEach((link) => {
        ...
        if (
          text.includes('القضايا') ||
          text.includes('Cases') ||
          text.includes('العملاء') ||
          text.includes('Clients') ||
          text.includes('الجلسات') ||
          text.includes('Hearings')
        ) {
      ...
      const mainContent = document.querySelector(
        'main, [role="main"], [class*="main"], [class*="content"]'
      );
    ...
    const reactComponents = await page.evaluate(() => {
      ...
      const reactElements = document.querySelectorAll(
        '[data-reactroot], [data-react-helmet], [class*="react"]'
      );
      const componentElements = document.querySelectorAll(
        '[class*="component"], [class*="page"], [class*="container"]'
      );
