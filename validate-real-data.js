...
async function validateAPI() {
  ...
  try {
    ...
    if (clientsData.success && clientsData.data.data) {
      ...
      console.log(
        `   🎯 Real data indicators: ${realDataCount}/${Math.min(3, clients.length)} clients\n`
      );
    } else {
    ...
    if (casesData.success && casesData.data.data) {
      ...
      cases.slice(0, 3).forEach((caseItem, index) => {
        console.log(
          `      ${index + 1}. Case ${caseItem.matter_id}: ${caseItem.matter_en || caseItem.matter_ar}`
        );
        if (caseItem.matter_court) console.log(`         🏛️ Court: ${caseItem.matter_court}`);
        if (caseItem.matter_start_date)
          console.log(`         📅 Start: ${caseItem.matter_start_date}`);
      });
      console.log('');
    } else {
    ...
    if (lawyersResponse.ok) {
      ...
      if (lawyersData.success && lawyersData.data) {
        const lawyers = Array.isArray(lawyersData.data)
          ? lawyersData.data
        ...
        lawyers.slice(0, 3).forEach((lawyer, index) => {
          console.log(
            `      ${index + 1}. ${lawyer.name || lawyer.full_name_en || lawyer.full_name_ar || 'Lawyer ' + lawyer.id}`
          );
          if (lawyer.specialization)
            console.log(`         🏷️ Specialization: ${lawyer.specialization}`);
