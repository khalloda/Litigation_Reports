...
test.describe('Comprehensive Login to Logo Upload Flow', () => {
  test('should complete full flow from login to logo upload and client save', async ({ page }) => {
    ...
    const addClientButton = page.locator(
      'button:has-text("Add Client"), button:has-text("إضافة عميل")'
    );
    ...
    if (await addClientButton.isVisible()) {
      ...
      const clientNameAr = page.locator(
        'input[name="client_name_ar"], input[placeholder*="اسم العميل"]'
      );
      const clientNameEn = page.locator(
        'input[name="client_name_en"], input[placeholder*="Client Name"]'
      );
