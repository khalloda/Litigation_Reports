...
test.describe('RTL and Mixed Content Testing', () => {
  ...
  test('should handle Arabic number formatting', async ({ page }) => {
    ...
    const numberInputs = page.locator(
      'input[type="number"], input[name*="amount"], input[name*="price"]'
    );
  ...
  test('should handle RTL in modal dialogs', async ({ page }) => {
    ...
    const modalTrigger = page
      .locator('[data-bs-toggle="modal"], [data-testid="modal-trigger"]')
      .first();
    if ((await modalTrigger.count()) > 0) {
      ...
      const modalButtons = modal.locator(
        '.modal-footer button, [data-testid="modal-actions"] button'
      );
  ...
  test('should handle RTL in file upload areas', async ({ page }) => {
    ...
    const fileUpload = page.locator(
      '[data-testid="file-upload"], .file-upload, input[type="file"]'
    );
