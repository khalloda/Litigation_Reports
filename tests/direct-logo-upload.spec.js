...
test.describe('Direct Client Logo Upload Test', () => {
  test('should test logo upload by directly navigating to clients page', async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("Add"), .btn-primary:has-text("Add"), [data-testid="add-client-btn"], button:has-text("إضافة"), button:has-text("Add Client")'
    );
    ...
    const arabicNameInput = page.locator(
      'input[placeholder*="اسم العميل بالعربية"], input[placeholder*="Arabic client name"], input[name="client_name_ar"], input[data-testid="client-name-ar"]'
    );
    ...
    // Fill English name
    const englishNameInput = page.locator(
      'input[placeholder*="اسم العميل بالإنجليزية"], input[placeholder*="English client name"], input[name="client_name_en"], input[data-testid="client-name-en"]'
    );
    ...
    const lawyerInput = page.locator(
      'input[placeholder*="المحامي"], input[placeholder*="lawyer"], input[name="contact_lawyer"], input[data-testid="contact-lawyer"]'
    );
    ...
    const filePreview = page.locator(
      'img[src*="blob:"], .logo-preview, .file-preview, img[alt*="preview"]'
    );
    ...
    const saveButton = page.locator(
      'button:has-text("Save"), .btn-primary:has-text("Save"), button[type="submit"], button:has-text("حفظ")'
    );
    ...
    const modalStillOpen = page.locator(
      '.modal.show, [role="dialog"]:not([style*="display: none"])'
    );
