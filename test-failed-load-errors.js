  ...
  try {
    ...
    const pageState = await page.evaluate(() => {
      ...
      const emptyStates = document.querySelectorAll(
        '[class*="empty"], [class*="no-data"], [class*="no-cases"], [class*="no-clients"], [class*="no-hearings"]'
      );
    ...
    if (pageState.failedTexts.length > 0) {
      pageState.failedTexts.forEach((failed, index) => {
        console.log(
          `         ${index + 1}. "${failed.text}" (${failed.tagName}.${failed.className})`
        );
      });
    }
    ...
    Object.entries(apiTest).forEach(([endpoint, result]) => {
      if (result.success) {
        console.log(
          `      ✅ ${endpoint}: Status ${result.status}, Count: ${result.dataCount}, Has Data: ${result.hasData}`
        );
        if (!result.hasData) {
    ...
    networkRequests.forEach((req, index) => {
      console.log(
        `      ${index + 1}. ${req.method} ${req.url} - Status: ${req.status || 'Pending'}`
      );
    });
    ...
    try {
      const casesLink = page
        .locator(
          'a[href*="cases"], [data-testid*="cases"], button:has-text("القضايا"), button:has-text("Cases")'
        )
        .first();
    ...
    if (failedLoadMatches) {
      console.log(
        `   📊 Found ${failedLoadMatches.length} "Failed to load" text instances in HTML`
      );
    } else {
