...
export function ServerPaginatedTable<T>({
  ...
  const renderPagination = () => {
    ...
    for (let i = startPage; i <= endPage; i++) {
      items.push(
        <Pagination.Item key={i} active={currentPage === i} onClick={() => onPageChange(i)}>
          {i}
        </Pagination.Item>
      );
    }
    ...
    return (
      <div className='d-flex justify-content-between align-items-center'>
        ...
        <Pagination dir={isRTL ? 'rtl' : 'ltr'} className={clsx('mb-0', isRTL && 'pagination-rtl')}>
          <Pagination.First
  ...
  return (
    <div className={clsx('server-paginated-table', className)}>
      <Table responsive striped hover className='mb-0' dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {totalPages > 1 && <div className='table-pagination mt-3'>{renderPagination()}</div>}
    </div>
  );
}
