...
test.describe('Debug Login Issue', () => {
  test('should debug login process step by step', async ({ page }) => {
    ...
    const loginRequests = requests.filter(
      (req) =>
        req.url.includes('/auth/login') || req.url.includes('/login') || req.method === 'POST'
    );
    ...
    const loginResponses = responses.filter(
      (resp) => resp.url.includes('/auth/login') || resp.url.includes('/login')
    );
    ...
    const redirectedToLogin =
      clientsUrl.includes('/login') || clientsUrl === 'http://localhost:3001/';
