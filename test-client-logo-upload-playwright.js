...
test.describe('Client Logo Upload Test', () => {
  test('should upload logo file to existing client "Sarieldin and Partners"', async ({ page }) => {
    ...
    const loginForm = page.locator(
      'form[data-testid="login-form"], .login-form, input[type="email"]'
    );
    ...
    await page.click(
      'a[href*="clients"], [data-testid="clients-link"], .nav-link:has-text("Clients")'
    );
    await page.waitForLoadState('networkidle');
    ...
    if ((await clientRow.count()) === 0) {
      ...
      await page.click(
        'button:has-text("Add"), .btn-primary:has-text("Add"), [data-testid="add-client"]'
      );
      await page.waitForLoadState('networkidle');

      // Fill client form
      await page.fill(
        'input[name="client_name_ar"], [data-testid="client-name-ar"]',
        'Sarieldin and Partners'
      );
      await page.fill(
        'input[name="client_name_en"], [data-testid="client-name-en"]',
        'Sarieldin and Partners'
      );
      ...
      await page.click(
        'button:has-text("Save"), .btn-primary:has-text("Save"), [data-testid="save-client"]'
      );
      await page.waitForLoadState('networkidle');
    ...
    const fileInput = page.locator(
      'input[type="file"], input[accept*="image"], [data-testid="logo-upload"]'
    );
  ...
  test('should handle file upload errors gracefully', async ({ page }) => {
    ...
    try {
      ...
      const errorMessage = page.locator(
        '.alert-danger, .text-danger, [data-testid="file-size-error"]'
      );
