...
class Hearing {
    ...
    public static function findById($id) {
        ...
        if ($hearing) {
            ...
            $hearing['lawyers'] = self::getLawyers($id);
        }
    ...
    public static function create($data, $lawyerIds = []) {
        $db = Database::getInstance();
        ...
        $hearingId = $db->insert(self::$table, $data);

        // Save lawyer relationships if provided
        if (!empty($lawyerIds) && is_array($lawyerIds)) {
            self::saveLawyers($hearingId, $lawyerIds);
        }
    ...
    public static function saveLawyers($hearingId, $lawyerIds) {
        error_log("Hearing::saveLawyers called with hearingId: $hearingId, lawyerIds: " . json_encode($lawyerIds));

        if (!is_array($lawyerIds) || empty($lawyerIds)) {
            error_log("Hearing::saveLawyers - No lawyer IDs provided or invalid format");
            return;
        }
        ...
        try {
            // First, delete existing relationships for this hearing
            $deleteResult = $db->delete('hearing_lawyers', 'hearing_id = :hearing_id', ['hearing_id' => $hearingId]);
            error_log("Hearing::saveLawyers - Deleted existing relationships: " . ($deleteResult ? 'success' : 'failed'));

            // Then, insert new relationships
            $insertCount = 0;
            foreach ($lawyerIds as $lawyerId) {
                if (is_numeric($lawyerId)) {
                    $insertResult = $db->insert('hearing_lawyers', [
                        'hearing_id' => $hearingId,
                        'lawyer_id' => $lawyerId
                        // created_at and updated_at are auto-generated by database
                    ]);
                    error_log("Hearing::saveLawyers - Inserted relationship hearing_id: $hearingId, lawyer_id: $lawyerId, result: " . ($insertResult ? 'success' : 'failed'));
                    if ($insertResult) $insertCount++;
                } else {
                    error_log("Hearing::saveLawyers - Skipped non-numeric lawyer ID: " . json_encode($lawyerId));
                }
            }
            error_log("Hearing::saveLawyers - Successfully inserted $insertCount lawyer relationships");
        } catch (Exception $e) {
            error_log("Hearing::saveLawyers - Error: " . $e->getMessage());
            throw $e;
        }
    }

    public static function getLawyers($hearingId) {
        $db = Database::getInstance();
        return $db->fetchAll("
            SELECT l.*, hl.created_at as assigned_at
            FROM lawyers l
            INNER JOIN hearing_lawyers hl ON l.id = hl.lawyer_id
            WHERE hl.hearing_id = :hearing_id
